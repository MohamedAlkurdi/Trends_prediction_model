...
from facebook_prophet.model_builder import build_model
...
def build_country_topic_model(country,topic):
    ...
    country_trend_data_path = model_data['country_trend_data_path']
    regerssors = model_data['regressors']

    print("from build_country_topic_model country_trend_data_path:",country_trend_data_path)
    print("from build_country_topic_model regerssors:",regerssors)

    model, forecasting_result, error_metrics, test_subset = build_model(country_trend_data_path, regerssors)
    
    # Reset the index to turn the index (date) into a column
